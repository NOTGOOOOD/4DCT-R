...
class VoxNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.decv1(F.interpolate(x, size=h4.shape[2:], mode='trilinear'))
        d5 = x
        x = torch.cat([x, h4], dim=1)
        del h4
        x = self.decv2(F.interpolate(x, size=h3.shape[2:], mode='trilinear'))
        d4 = x
       
        x = torch.cat([x, h3], dim=1)
        del h3
        x = self.decv3(F.interpolate(x, size=h2.shape[2:], mode='trilinear'))
        d3 = x

        x = torch.cat([x, h2], dim=1)
        del h2
        x = self.decv4(F.interpolate(x, size=h1.shape[2:], mode='trilinear'))
        d2 = x
